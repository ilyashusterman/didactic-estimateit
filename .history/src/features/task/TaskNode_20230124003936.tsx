    ...
    return (
        <div style={{ background: '#717EC3', borderRadius: '10px' }}>
            ...
            <div style={{ marginTop: '20px' }}>
                {props.inputs.map((port) => React.cloneElement(port, {
                    style: { width: '50px', height: '25px', background: '#1B263B' }
                }))}
                {props.outputs.map((port) => React.cloneElement(port, {
                    style: { width: '50px', height: '25px', background: '#1B263B' }
                }))}
            </div>
        </div>
...
const UncontrolledDiagram: React.FC<UncontrolledDiagramProps> = ({ tasks }) => {
    let links: any[]= []
